      ...
      <div className="p-4 lg:p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="stat-card bg-white rounded-xl p-4 lg:p-6 shadow-sm">
            ...
            <p className="text-2xl lg:text-3xl font-bold text-gray-900">{formatPrice(riepilogo.entrate)}</p>
            <p className="text-xs lg:text-sm text-gray-500">Entrate stagione</p>
          </div>
          ...
          <div className="stat-card bg-white rounded-xl p-4 lg:p-6 shadow-sm">
            ...
            <p className="text-xs lg:text-sm text-gray-500">Netto stagione</p>
          </div>
          ...
          <div className="stat-card bg-white rounded-xl p-4 lg:p-6 shadow-sm">
            ...
            <p className="text-2xl lg:text-3xl font-bold text-gray-900">{riepilogo.prenotazioni}</p>
          ...
          <div className="stat-card bg-white rounded-xl p-4 lg:p-6 shadow-sm">
            ...
            <p className="text-xs lg:text-sm text-gray-500">Media prenotazione</p>
          </div>
        </div>
        ...
        {haData && (
          <div className="grid lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white rounded-xl p-4 lg:p-6 shadow-sm">
              <h3 className="font-semibold text-gray-900 mb-4">Andamento Entrate Stagione {stagione.anno}</h3>
            ...
            {/* Fonti Prenotazioni */}
            <div className="bg-white rounded-xl p-4 lg:p-6 shadow-sm">
              <h3 className="font-semibold text-gray-900 mb-4">Entrate per Fonte</h3>
                  ...
                  <div className="flex-1 space-y-2">
                    {categorieEntrate.map((cat) => (
                      <div key={cat.nome} className="flex items-center justify-between">
                        ...
                        <span className="text-sm font-medium text-gray-900">{formatPrice(cat.valore)}</span>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        {pagamentiInAttesa.length > 0 && (
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="font-semibold text-amber-800">Pagamenti in Attesa ({pagamentiInAttesa.length})</h3>
            </div>
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {pagamentiInAttesa.map((pag) => (
                <div key={pag.id} className="bg-white rounded-lg p-3 border border-amber-200">
                  <div className="flex items-center justify-between mb-2">
                    <span className="font-medium text-gray-900">{pag.ospite}</span>
                    <span className="text-xs px-2 py-1 bg-amber-100 text-amber-700 rounded-full capitalize">
                      {pag.tipo}
                    </span>
                  </div>
                  <p className="text-lg font-bold text-gray-900">{formatPrice(pag.importo)}</p>
                  <p className="text-xs text-gray-500">Check-in: {formatDate(pag.scadenza)}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm">
            <div className="p-4 lg:p-6 border-b border-gray-100">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <h3 className="font-semibold text-gray-900">Ultime Prenotazioni</h3>
                  ...
                  <div className="flex items-center gap-1 bg-gray-100 p-1 rounded-lg">
                    <button
                      ...
                      className={cn(
                        ...
                        filterTipo === 'tutti' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-600'
                      )}
                    >
                      Tutti
                    </button>
                    <button
                      ...
                      className={cn(
                        'px-3 py-1.5 text-sm font-medium rounded-md transition-colors',
                        filterTipo === 'entrate' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-600'
                      )}
                    >
                      Entrate
                    </button>
                ...
                <div key={mov.id} className="p-4 hover:bg-gray-50 transition-colors">
                  <div className="flex items-center gap-4">
                    <div className={cn(
                      ...
                      mov.tipo === 'entrata' ? 'bg-green-100' : 'bg-red-100'
                    )}>
                      {mov.tipo === 'entrata' ? (
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-medium text-gray-900 truncate">{mov.descrizione}</p>
